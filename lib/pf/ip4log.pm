...
sub ip4log_db_prepare {
    ...
    $ip4log_statements->{'ip4log_view_by_ip_sql'} = get_db_handle()->prepare(
        qq [ SELECT * FROM ip4log WHERE ip = ? AND (end_time = 0 OR ( end_time + INTERVAL 30 SECOND ) > NOW()) ORDER BY start_time DESC LIMIT 1 ]
    );
    ...
    $ip4log_statements->{'ip4log_view_by_mac_sql'} = get_db_handle()->prepare(
        qq [ SELECT * FROM ip4log WHERE mac = ? AND (end_time = 0 OR ( end_time + INTERVAL 30 SECOND ) > NOW()) ORDER BY start_time DESC LIMIT 1 ]
    );

    $ip4log_statements->{'ip4log_list_open_sql'} = get_db_handle()->prepare(
        qq [ SELECT * FROM ip4log WHERE end_time=0 OR end_time > NOW() ]
    );

    $ip4log_statements->{'ip4log_list_open_by_ip_sql'} = get_db_handle()->prepare(
        qq [ SELECT * FROM ip4log WHERE ip = ? AND (end_time = 0 OR end_time > NOW()) ORDER BY start_time DESC ]
    );

    $ip4log_statements->{'ip4log_list_open_by_mac_sql'} = get_db_handle()->prepare(
        qq [ SELECT * FROM ip4log WHERE mac = ? AND (end_time = 0 OR end_time > NOW()) ORDER BY start_time DESC ]
    );
    ...
    $ip4log_statements->{'ip4log_rotate_insert_sql'} = get_db_handle()->prepare(
        qq [ INSERT INTO ip4log_archive SELECT * FROM ip4log_history WHERE end_time < DATE_SUB(?, INTERVAL ? SECOND) LIMIT ? ]
    );
